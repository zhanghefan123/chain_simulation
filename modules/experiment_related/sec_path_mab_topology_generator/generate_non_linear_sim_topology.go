...
// WriteNonLinearTopologyDescription writes the simulator topology description as JSON.
func (osmdTopologyDescription *NonLinearTopologyDescription) WriteNonLinearTopologyDescription(destinationFile string) error {
	bytes, err := json.MarshalIndent(osmdTopologyDescription, "", " ")
	if err != nil {
		return fmt.Errorf("marshal non-linear topology description: %w", err)
	}
	if err := file.WriteStringIntoFile(destinationFile, string(bytes)); err != nil {
		return fmt.Errorf("write non-linear topology description to %s: %w", destinationFile, err)
	}
	return nil
}
