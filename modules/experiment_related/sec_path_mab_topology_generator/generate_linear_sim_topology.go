...
// WriteLinearTopologyDescription writes the simulator topology description as JSON.
func (linearTopologyDescription *LinearTopologyDescription) WriteLinearTopologyDescription(destinationFile string) error {
	bytes, err := json.MarshalIndent(linearTopologyDescription, "", " ")
	if err != nil {
		return fmt.Errorf("marshal linear topology description: %w", err)
	}
	if err := file.WriteStringIntoFile(destinationFile, string(bytes)); err != nil {
		return fmt.Errorf("write linear topology description to %s: %w", destinationFile, err)
	}
	return nil
}
