...
var (
	...
	resultScenarioPrefix    = "path_mab/frequency_0_1s/delay_1_25ms"
	sourceNodeIndex         = 1
	numberOfEpochs          = 500
	...
	secPathMabType          = types.SecPathMabStrategy_PATH_MAB
...
func GenerateSecPathMabPathMabDifferentBatchSizeEvents(currentExperimentIndex int, setting *entities.ConfigurationSetting) ([]*entities.Event, error) {
	...
	initOsmdEvent := &entities.Event{
		...
		Handler: func() error {
			go func() {
				...
				err = validation_manager.InitOsmd(sourceNodeIndex, &entities.InitOsmd{
					LearningRate:            learningRate,
					MinimumDeliveryRatio:    minimumDeliveryRatio,
					DestinationPort:         destinationPort,
					Destinations:            destinations,
					MessageSize:             messageSize,
					NumberOfPktsPerLink:     numberOfPacketsPerLink,
					MiniBatchSize:           miniBatchSize,
					PacketSendingInterval:   interval,
					SecPathMabStrategy:      int(secPathMabType),
	...
	maliciousSeed := executor.MaliciousSeedFromSetting(setting)
	...
	secPathMabBatchEvents = append(secPathMabBatchEvents, stopEvent)
...
func ChangeCorruptRatioInTimStampLevel(seed int64) error {
	return executor.ScheduleCorruptRatioChanges(seed, executor.Corruptratioscheduleparamsfrequency01s)
}

// SecPathMabPathMabDifferentBatchSizeExperiment 进行多次的实验
func SecPathMabPathMabDifferentBatchSizeExperiment() error {
	return executor.RunDifferentBatchSizeExperiments(
		resultScenarioPrefix,
		onlineconfig.DifferentBatchSizeConfigurationSettings(resultScenarioPrefix, "5"),
		GenerateSecPathMabPathMabDifferentBatchSizeEvents,
	)
}
